...
const CartItem = ({cartitem,onUpdateCartQty,onUpdateColorSize,onUpdateMeasurement,onRemoveFromCart,onUpdateSelect}) => {
    ...
    const [imagepath]=useState(cartitem.product.product_type==='normal' ? `http://localhost:3002/server/uploads/products/${cartitem.product.image[0].filename}`:`http://localhost:3002/server/uploads/products/prefarestyleproducts/${cartitem.product.image[0].filename}`)
